...
@Component({
  ...
  templateUrl: './listado-todos-por-usuario.component.html',
  styleUrls: ['./listado-todos-por-usuario.component.css']
})
export class ListadoTodosPorUsuarioComponent implements OnInit {
  ...
  array_estados=[{id:1,name:'Vigentes'},{id:2,name:'Anteriores'}];
  frm={
    titulo:null,
    estado:1,
    fecha_inicio:null,
      ...
      .then(async (datos) => {
        ...
        if (datos && datos.data && datos.data.Data) {
          ...
          for (let i = 0; i < dt.length; i++) {
            ...
            e.today=today;
            let b64=await this.onGetSavedFileByCodeWithReturn(e.iden_imagen);
            dt[i].b64_file= b64;
