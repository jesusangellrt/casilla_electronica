...
import { AlertService, AuthService, EncuestaAgradecimientoData, EncuestaBienvenidaData, EncuestaData, EncuestaMainData, EncuestasService, FileService, TIPO_ARCHIVO_ENUMERADO, TIPO_ENCUESTA } from '@app/_core';
...
@Component({
  ...
  templateUrl: './vista-encuesta-ext.component.html',
  styleUrls: ['./vista-encuesta-ext.component.css']
})
export class VistaEncuestaExtComponent implements OnInit {
  ...
  show_preguntas_seccion = false;
  isCheckedAutorizo:boolean=false;
  constructor(
  ...
  goEncuesta=async ()=>{
    if(this.isCheckedAutorizo){
      ...
      await this.encuesta_s._GetPreguntasCondicionalesByEncuesta({ id_encuesta: this.id_encuesta, es_autorizado_datos: this.isCheckedAutorizo, es_actualizar: true})
      .then((datos) => {
        if (datos && datos.data) {
