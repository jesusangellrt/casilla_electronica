...
@Component({
  ...
  templateUrl: './plataforma-extranet.component.html',
  styleUrls: ['./plataforma-extranet.component.css']
})
export class PlataformaExtranetComponent implements OnInit {
  ...
  getConfiguracion_inicial(){
    this.configuracion_inicial_s.GetActualConfiguracion(TIPO_ENCUESTA.extranet).then(datos => {
      if(datos.data){
  ...
  save_update=()=>{
    ...
    if(allow){
      ...
      let input:ConfiguracionInicialSaveUpdateRequest={
        ...
        es_eliminado:false,
        user_name:'',
        tipo_encuesta: TIPO_ENCUESTA.extranet
      };
