...
@Component({
  ...
  templateUrl: './modal-registro-pregunta-extranet.component.html',
  styleUrls: ['./modal-registro-pregunta-extranet.component.css']
})
export class ModalRegistroPreguntaExtranetComponent implements OnInit {
  ...
  @Input() es_con_seccion:boolean=false;
  ...
  array_tipo_fecha: any[] = [];
  texto_mensaje: string=null;
  checkedFila:boolean=false;
  checkedColumna:boolean=false;
  ...
  async ngOnInit() {
    ...
    await this.getCategoriaExtranetList();
    await this.getSubCategoriaExtranetList();
    await this.getSeccionList();
    ...
    if(this.frm.id_pregunta!=null &&this.frm.id_pregunta>0){
      ...
      if(pregunta.id_tipo_pregunta == TIPO_PREGUNTA.archivo){
        if(pregunta.json_archivo_tipo_archivo != null && pregunta.json_archivo_tipo_archivo != undefined){
          ...
          await this.getArchivoTipoArchivo();
        }
      }
      
      if(pregunta.id_tipo_pregunta == TIPO_PREGUNTA.fecha){
  ...
  JSON_INIT(){
    ...
    if(id_tipo_pregunta == TIPO_PREGUNTA.numerico_entero){
      this.JSON_NUMERICO_ENTERO = json;
    }
    if(id_tipo_pregunta == TIPO_PREGUNTA.texto){
      this.JSON_NUMERICO_TEXTO = json;
    }
    if(id_tipo_pregunta==TIPO_PREGUNTA.mensaje){
  ...
  frmPRG={
    ...
    frmTipoArchivo:new FormControl(null),
    frmEsCondicionada: new FormControl(false),
    frmEsCondicional: new FormControl(false),
    frmEsAgregarAlternativa: new FormControl(false),
    frmPreguntaCondicional: new FormControl(null),
    frmEsFinalizaEncuesta:new FormControl(false),
    frmMensaje: new FormControl(null),
    frmSeccion:new FormControl(null)
  }
  ...
  async getSeccionList(){
    await this.encuesta_s._getSeccionList(this.id_encuesta).then(datos => {
      this.array_seccion =datos.data.filter(x=>x.es_eliminado==false);
    }).catch(err => {});
  }

  array_categoria=[];
  async getCategoriaExtranetList(){
    await this.categoria_s._getCategoriaExtranetList().then(datos => {
      this.array_categoria =datos.data.filter(x=>x.es_eliminado==false);
    }).catch(err => {});
  }
  array_sub_categoria=[];
  array_sub_categoria_all=[];
  async getSubCategoriaExtranetList(){
    await this.categoria_s._getSubCategoriaExtranetList().then(datos => {
  ...
  async getTipoFecha(){
    await this.sis_enumerado_s.GetEnumeradoByTipoEnumerado(SIS_TIPO_ENUMERADO.tipo_fecha).then(datos => {
  ...
  JSON_SELECCION_ARCHIVO = JSON_TIPO_PREGUNTA.archivo;
  ...
  JSON_MENSAJE = JSON_TIPO_PREGUNTA.mensaje;
  ...
  url = "/api/comun/descargar-archivo";  _imagen_archivo: Array<any>;
  ...
  handleSaveFormularioModal=()=>{
    ...
    if(this.frm.id_tipo_pregunta==TIPO_PREGUNTA.archivo){
    ...
    if(this.frm.id_tipo_pregunta==TIPO_PREGUNTA.fecha){
      let total_fecha= 0;
      this.array_tipo_fecha.forEach(element => {
        if(element.selected){
          total_fecha++;
      ...
      if(total_fecha == 0){
        ...
        allow=false; has_tipo_fecha=false;
      }else{
        allow=true; 
    ...
    if(this.frm.id_tipo_pregunta==TIPO_PREGUNTA.mensaje){      
  ...
  _show_modal_matriz=(selected_tipo:number)=>{
    ...
    if(selected_tipo==TIPO_PREGUNTA.desplegable_institucional ||
      ...
      selected_tipo==TIPO_PREGUNTA.fecha ||
      selected_tipo==TIPO_PREGUNTA.archivo ||
      selected_tipo==TIPO_PREGUNTA.texto){
  ...
  onChangeFilaColumna(value: string){
    this.array_filas=[];
    this.array_columnas=[];
    this.frmFilaName = [];
    this.frmColumnaName = [];
