...
@Component({
  ...
  templateUrl: './datos-encuesta-extranet.component.html',
  styleUrls: ['./datos-encuesta-extranet.component.css']
})
export class DatosEncuestaExtranetComponent implements OnInit {
  ...
  tagsTitulo: Array<string> = [];
  public editorTitulo = DecoupledEditor;
  configEditorTitulo = DecoupledInit.configEditor;
  expresionTitulo = /\[\[[A-Z0-9\_]+\]\]/g;

  public onChangeTitulo({ editor }: ChangeEvent) {
    const data = editor.getData();
  ...
  save_update(){
    ...
    if(allow){
      let input:EncuestaSaveUpdateRequest={
        ...
        tipo_encuesta: TIPO_ENCUESTA.extranet//tipo de encuesta Extranet
      };
      this.spinner.show();
