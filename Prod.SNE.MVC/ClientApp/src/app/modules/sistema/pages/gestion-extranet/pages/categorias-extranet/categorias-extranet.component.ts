...
import {  ConfiguracionInicialService,  SisEnumeradoService, SIS_TIPO_ENUMERADO, 
  ...
  CategoriaService,
  CATEGORIA,
  CATEGORIA_EXTRANET} from "@app/_core";
...
@Component({
  ...
  templateUrl: './categorias-extranet.component.html',
  styleUrls: ['./categorias-extranet.component.css']
})
export class CategoriasExtranetComponent implements OnInit {
  ...
  selected_categoria:CATEGORIA_EXTRANET;
  allow_edit:boolean=true;
  title_modal:string='';
  show_save_edit=(itemCategoria:CATEGORIA_EXTRANET, allow_edits=true)=>{
    if(itemCategoria){
  ...
  _enable_disable=(id_categoria_extranet, future_status)=> {
    this.modal.confirm({
