...
export class EncuestasComponent implements OnInit {
  ...
  get_list=()=>{
    ...
    let filter = {
      ...
      id_tipo_encuesta: TIPO_ENCUESTA.intranet
    };
    this.spinner.show();
    this.encuesta_s._getEncuestaWithDependenciaList(filter, this.pageIndex, this.pageSize)
      .then((datos) => {
        this.spinner.hide();
  ...
  descargar_data_encuesta=async (_id_encuesta:number)=>{
    ...
    let cols=[
      { key: 'Nro', width: 12 },
      { key: 'Organo de alta dirección', width: 20 },
      { key: 'Dirección / Oficina', width: 20 },
      { key: 'Unidad Orgánica', width: 20 },
      { key: 'Edad', width: 12 },
      { key: 'Regimen', width: 15 },
      { key: 'Sexo', width: 15 },
    ];
    sheet.getCell('A1').value = "Nro";
    sheet.getCell('B1').value = "Organo de alta dirección";
    sheet.getCell('C1').value = "Dirección / Oficina";
    sheet.getCell('D1').value = "Unidad Orgánica";
    sheet.getCell('E1').value = "Edad";
    sheet.getCell('F1').value = "Regimen";
    sheet.getCell('G1').value = "Sexo";
    let _keycols=['A1', 'B1', 'C1', 'D1', 'E1','F1','G1'];
    let _header_cols_only=['A', 'B', 'C', 'D', 'E','F','G'];
    await this.encuesta_s._getRespuestasByEncuesta({id_encuesta: _id_encuesta}, 1, pageSize)
    .then((datos) => {
    ...
    for (let page_index = 1; page_index <= this.total_pages_descarga; page_index++) {
      await this.encuesta_s._getRespuestasByEncuesta({id_encuesta: _id_encuesta}, page_index, pageSize)
      .then((datos) => {
        if (datos && datos.data && datos.data.Data) {
          let dt = datos.data.Data;
          if(construir_header){
          ...
          for (let i = 0; i < dt.length; i++) {
            ...
            if(p1){
              ...
              let d3=this.array_dependencias.find(f=>f.id==p1.d3);

              sheet.getCell('B'+String(begin_fila)).value = d1?d1.nombre:null;
              sheet.getCell('C'+String(begin_fila)).value = d2?d2.nombre:null;
              sheet.getCell('D'+String(begin_fila)).value = d3?d3.nombre:null;
            }
            sheet.getCell('E'+String(begin_fila)).value = item.edad;
            sheet.getCell('F'+String(begin_fila)).value = item.regimen;
            sheet.getCell('G'+String(begin_fila)).value = item.sexo;
              ...
              if(respuesta_pregunta){
                if(respuesta_pregunta.id_tipo==TIPO_PREGUNTA.desplegable_institucional){
                  if(respuesta_pregunta.tbl_data==1){
                    ...
                    let d3=this.array_dependencias.find(f=>f.id==_rpta.d3);
    
                    sheet.getCell(col+String(begin_fila)).value =   'Organo de alta dirección: '+d1?d1.nombre:'';
                    sheet.getCell(col+String(begin_fila+1)).value = 'Dirección / Oficina: '+d2?d2.nombre:'';
                    sheet.getCell(col+String(begin_fila+2)).value = 'Unidad Organica: '+d3?d3.nombre:'';
                    conteo=3;
                  }else{
                ...
                else if(respuesta_pregunta.id_tipo==TIPO_PREGUNTA.seleccion_multiple
                  ...
                  respuesta_pregunta.respuesta.forEach(__fila=>{
                    ...
                    __fila.respuestas.forEach((_rpt,col_idx)=>{
                      ...
                      }else if(forma.id_tipo==TIPO_PREGUNTA.numerico_entero 
