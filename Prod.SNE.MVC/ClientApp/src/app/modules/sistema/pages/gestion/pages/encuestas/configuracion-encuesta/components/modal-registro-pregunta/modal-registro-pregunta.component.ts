...
@Component({
  selector: 'app-modal-registro-pregunta',
  templateUrl: './modal-registro-pregunta.component.html',
  styleUrls: ['./modal-registro-pregunta.component.css']
})
export class ModalRegistroPreguntaComponent implements OnInit {
  ...
  async getTipoPregunta(){
    await this.sis_enumerado_s.GetEnumeradoByTipoEnumerado(SIS_TIPO_ENUMERADO.tipo_pregunta).then(datos => {
      this.array_tipo_pregunta =datos.data.filter(x=>x.es_eliminado==false && x.id_enumerado != TIPO_PREGUNTA.mensaje);
    }).catch(err => {});
  }
  array_categoria=[];
  ...
  handleSaveFormularioModal=()=>{
    ...
    if(!allow){
      ...
      if(has_tipo_fecha==false){
        str+="<li>Debe seleccionar al menos una fecha.</li>";
      }
    }else{
      if(this.frm.id_tipo_pregunta == TIPO_PREGUNTA.matriz){
        let cols=[],rows=[];
  ...
  _show_modal_matriz=(selected_tipo:number)=>{
    ...
    if(this.selected_tipo_forma!=TIPO_PREGUNTA.numerico_entero 
      && this.selected_tipo_forma!=TIPO_PREGUNTA.texto
      && this.selected_tipo_forma!=TIPO_PREGUNTA.fecha){
