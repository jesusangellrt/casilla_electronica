...
import { CATEGORIA, CategoriaService } from '@app/_core';
import { NzModalService, NzNotificationService } from 'ng-zorro-antd';
import { NgxSpinnerService } from 'ngx-spinner';

@Component({
  selector: 'app-save-update-categoria',
  templateUrl: './save-update-categoria.component.html',
  styleUrls: ['./save-update-categoria.component.css']
})
export class SaveUpdateCategoriaComponent implements OnInit {
  ...
  handleSaveFormularioModal=()=>{
    if(this.frmPRG.frmNombre.valid){
      if(this.array_sub_categorias.filter(x=>x.nombre==null||x.nombre=='').length==0){
        ...
        filter.array_sub_categorias = this.array_sub_categorias.map(m=>{return { 
